...
def remove_urls(texts):
    ...
    texts_clean = texts.str.replace("^\.{3}", "", regex=True).str.replace(
        "\.{3}", ".", regex=True
    )
...
def tfidf_cleanup(texts):
    ...
    texts = texts.str.replace(r"5M \\xc5\\x5", "5 Euro", regex=False)
    texts = texts.str.replace(r"Millionen \\xc5\\x5", "Millionen Euro", regex=False)
    texts = texts.str.replace(r"Milliarden \\xc5\\x5", "Millionen Euro", regex=False)
    ...
    texts = (
        texts.str.replace("xcx", "", regex=False)
...
class BasicTfidf:
    def __init__(
        ...
        self.sw = self._retrieve_stopwords(path=stopwords_file)
    ...
    def load(self, path: str, zip_archive: zipfile.ZipFile = None):
    ...
    def stop_words_(self):
        return list(set(self.sw + list(self.vectorizer.steps[-1][1].stop_words_)))
