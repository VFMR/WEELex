...
class AugmentedEnsemble(BaseEnsemble, BaseEstimator):
    ...
    @staticmethod
    def _get_drawclass_data(
        array: Union[list, pd.DataFrame, np.ndarray],
        targets: np.ndarray,
        classvalue: int,
    ) -> np.ndarray:
    ...
    def _random_category(array: List[str], cat: str) -> str:
        ...
        Returns:
            str: selected item
        """
        random_cat = random.choice([x for x in array if x != cat])
        return random_cat
    ...
    def _getkeeps(
        ...
        if classvalue == 1:
            ...
            random_cat = self._random_category(
                array=self.outside_categories_all, cat=self.category
            )
    ...
    def draw_random_samples_classwise(
        self,
        X: pd.DataFrame,
        y: pd.DataFrame,
        classvalue: int,
        ...
        if include_all_original:
            ...
            if classvalue == 0:
                ...
                # inside category
                random_ix = [
                    random.randint(0, len(X) - 1) for _ in range(n_original_samples)
                ]
    ...
    def fit(self, X, y):
        ...
        X1, y1 = self.draw_random_samples_classwise(
            X, y, classvalue=1, n_samples_multiplier=ns_m, include_all_original=True
        )
        ...
        for model in self.progress_func(self.models):
            ...
            # random shuffling:
            X_train, _, y_train, __ = train_test_split(X_train, y_train, test_size=2)
...
class FullEnsemble(BaseEnsemble):
    ...
    def load(self, path):
        ...
        mnames = glob.glob(os.path.join(path, f"liwc_ensemble_{self.category}*"))
        mnames = [x for x in mnames if "model" not in x.replace("liwc_models", "")]
        trained_models = []
        for name in mnames:
...
def ensemble_cross_val_score(model, X, y, cv=5):
    ...
    for i in range(len(X_splits)):
        X_train = pd.DataFrame(
            np.concatenate([x for j, x in enumerate(X_splits) if j != i])
        )
        y_train = pd.DataFrame(
            np.concatenate([x for j, x in enumerate(y_splits) if j != i])
        )
